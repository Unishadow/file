...
function ENT:SpawnFunction(pl, tr)
    local e = ents.Create("atcarrier"); -- This should be the same name as the file
	local spawn_height = 6; -- How high above the ground the vehicle spawns. Change if it's spawning too high, or spawning in the ground.
...
function ENT:Initialize()
    ...
    self.WeaponLocations = {
        Right = self:GetPos() + self:GetForward() * 730 + self:GetRight() * 45 + self:GetUp() * 50,
	...
	self.ExitModifier = {x=175,y=600,z=100}
...
function ENT:Effects()
	...
	self.EnginePos = {
		self:GetPos()+self:GetForward()*-480+self:GetUp()*445+self:GetRight()*-110,
		self:GetPos()+self:GetForward()*-480+self:GetUp()*445+self:GetRight()*110,
	}
    ...
    local function CalcView()      
		...
		local p = LocalPlayer();
		local self = p:GetNetworkedEntity("ATCarrier", NULL)
		if(IsValid(self)) then
			local fpvPos = self:GetPos(); -- This is the position of the first person view if you have it
			View = SWVehicleView(self,950,700,fpvPos);		-- 700 is distance from vehicle, 200 is the height.
			return View;
		end
    end
    hook.Add("CalcView", "ATCarrierview", CalcView) -- This is very important. Make sure the middle arguement is unique. In this case the ship name + view
 
	local function ATCarrierReticle()
		
		local p = LocalPlayer();
		local Flying = p:GetNWBool("FlyingATCarrier");
		local self = p:GetNWEntity("ATCarrier");
    ...
    hook.Add("HUDPaint", "ATCarrierReticle", ATCarrierReticle) -- Here you need to make the middle argument something unique again. I've set it as what the function is called. Could be anything. And the final arguement should be the function just made.
